...
auroc(params$after_splsda$splsda, roc.comp = 1)
```


```{r, eval=params$input$draw_cim}
conds = levels(factor(params$classes))
cond.col = c('red', 'green')
names(cond.col) = conds
cim(params$after_splsda$splsda, row.sideColors = cond.col[params$classes], legend = list())
```


```{r, eval=params$input$draw_loadings}
plotLoadings(params$after_splsda$splsda, contrib = 'max', method = 'mean')
```